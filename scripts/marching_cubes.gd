class_name MarchingCubes extends RefCounted
...
func _init(ps: Vector3i, pe: Vector3i, step: int):
...
func _step(f: float, s: int):
	if s > 0:
		return round(f * s) / s
	return f

func _generate_cell(points: Dictionary, vec: Vector3, lod: int):
	var c = 0
	var f = [null, null, null, null, null, null, null, null]
	for i in f.size():
		f[i] = points[vec + VERTICES[i] * lod]
		if f[i] > 0:
			c += 1 << i
			_pending = true
	...
	for face in CASES[c]:
		for edge in face:
			var e0 = EDGES[edge][0]
			var e1 = EDGES[edge][1]
			var v0 = VERTICES[e0] * lod
			var v1 = VERTICES[e1] * lod
			var f0 = f[e0]
			var f1 = f[e1]
			var t0 = _step(1 - (0 - f0) / (f1 - f0), steps)
			var t1 = 1 - t0
			var result = vec + (v0 * t0 + v1 * t1)
			var uv = Vector2(
				(result.x - mins.x) / (maxs.x - mins.x),
				(result.z - mins.z) / (maxs.z - mins.z)
			)

			_surface_tool.set_uv(uv)
			_surface_tool.add_vertex(result)

func _generate(points: Dictionary, lod: int):
	var arr_mesh = ArrayMesh.new()
	_surface_tool.begin(Mesh.PRIMITIVE_TRIANGLES)

	for x in range(mins.x, maxs.x, lod):
		for y in range(mins.y, maxs.y, lod):
			for z in range(mins.z, maxs.z, lod):
				_generate_cell(points, Vector3(x, y, z), lod)
...
func get_mesh(points: Dictionary, lod: int):
	return _generate(points, lod)
